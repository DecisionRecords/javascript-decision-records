...
function _copyTemplateIn(
  ...
  let source_template_file = path.resolve(path.join(sourcePath, templateFile + '.' + systemLanguage + '.' + templateType));
  let source_language_file = path.resolve(path.join(sourcePath, templateFile + '.' + systemLanguage + '.ref'));
  let destination_template_file = path.resolve(path.join(workingDir, templateDir, templateFile + '.' + systemLanguage + '.' + templateType));
  let destination_language_file = path.resolve(path.join(workingDir, templateDir, templateFile + '.' + systemLanguage + '.ref'));
...
export default function decisionRecordInit(
  ...
  let decisionRecordConfig = new DecisionRecordConfig(workingDir);
