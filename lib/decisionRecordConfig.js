    ...
    if (this.recordRoot !== "." && workingDir !== cwd()) {
      ...
      while (this.recordRoot == '.' && directory != '/' && !directory.match(/^[A-Za-z]:\\$/)) {
        ...
        if (existsSync(path.join(directory, '.adr-dir'))) {
          let _file = readFileSync(path.join(directory, '.adr-dir'), 'utf8');
          if (_file.length > 0) {
            _file = path.normalize(_file.replace(/\\/g, '/'));
            let _recordPath = _file.split(/\r?\n/);
        ...
        if (existsSync(path.join(directory, '.decisionrecords-config'))) {
          let _recordPath = path.join('doc', 'decision_records');
          let _templateDirPath = '';
          let _configFile = path.join(directory, '.decisionrecords-config');
          let _file = readFileSync(_configFile, 'utf8');

          let _lines = _file.split(/\r?\n/);
          for (var _line = 0; _line < _lines.length; _line++) {
            let _parse = _lines[_line].match(/^records=(.*)$/);
            if (_parse !== null) {
              _recordPath = path.normalize(_parse[1]);
            }
            _parse = _lines[_line].match(/^language=(.*)$/);
            if (_parse !== null) {
              this.systemLanguage = _parse[1];
            }
            _parse = _lines[_line].match(/^dateOnStatus=(.*)$/);
            if (_parse !== null) {
            ...
            _parse = _lines[_line].match(/^templateDir=(.*)$/);
            if (_parse !== null) {
              _templateDirPath = path.normalize(_parse[1]); // TODO: Untested Line
            }
            _parse = _lines[_line].match(/^template=(.*)$/);
            if (_parse !== null) {
              this.templateFile = _parse[1];
            }
            _parse = _lines[_line].match(/^fileType=(.*)$/);
            if (_parse !== null) {
      ...
      if (this.recordRoot !== '.' && existsSync(this.recordRoot)) {
        ...
        if (this.fullTemplateReferencePath.length > 0) {
          _file = readFileSync(this.fullTemplateReferencePath, 'utf8');
          _lines = _file.split(/\r?\n/);
          for (var _line = 0; _line < _lines.length; _line++) {
            _parse = _lines[_line].match(/^([^=]+)="(.*)"$/)
